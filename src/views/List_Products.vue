<template>
  <main class="container mx-auto my-8">
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <router-link v-for="product in products" :key="product._id" :to="{ name: 'details', params: { id: product._id }, query: { imageUrl: getImageUrl(product) } }">
        <div class="bg-white p-4 rounded shadow-md cursor-pointer">
          ...
          <button @click="addToCart(product)" class="mt-4 bg-blue-500 text-white px-4 py-2 rounded">Add to Cart</button>
        </div>
      </router-link>
    </section>
  </main>
